...
void la_media_adv_format_text(la_vstring * const vstr, void const * const data, int indent) {
	...
	// Prepare time
	LA_ISPRINTF(vstr, indent, "Link %s was %s at %s:%s:%s UTC\n", get_link_description(msg->current_link[0]), (msg->state[0] == 'E') ? "established" : "lost", msg->hour, msg->minute, msg->second);
	...
	for(int i=0; i<count; i++) {
	  ...
	  if (i == count - 1) {
	    LA_ISPRINTF(vstr, indent, "%s", link);
	  } else {
	    LA_ISPRINTF(vstr, indent, "%s, ", link);
	  }
	}
